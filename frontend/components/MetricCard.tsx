...
function ComparisonCard({
  ...
  const maxValue = Math.max(spain, argentina, 1);
  const spainWidth = Math.min(100, (spain / maxValue) * 100);
  const argentinaWidth = Math.min(100, (argentina / maxValue) * 100);
  const leader = spain > argentina ? "Spain" : argentina > spain ? "Argentina" : "Equal";

  const leaderColor =
    leader === "Spain" ? "text-red-300" : leader === "Argentina" ? "text-sky-300" : "text-gray-300";

  return (
    <div className="bg-white/10 backdrop-blur-xl border border-white/10 rounded-3xl p-5 shadow-xl hover:scale-[1.03] transition">
      ...
      <div className={`text-xs mb-5 font-bold ${leaderColor}`}>
        Leader:
        <span className="ml-1">{leader === "Equal" ? "Draw" : leader}</span>
      </div>
